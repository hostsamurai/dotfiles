...
local translator = require 'makyo.plugins.translator'

-- Utilities -----------------------------------------------------------

local function map(mode, noremap, cmd)
  return function (chord)
    ...
    vimp[mode .. m](opts, '<leader>' .. chord, cmd)
  end
end

local n  = M.partial(map, 'n', false)
local nn = M.partial(map, 'n', true)
local vn = M.partial(map, 'v', true)
...
local function setup()
  local leader_map = translator.init(map_tree)
  ...
  M(ignore_list):each(function(k) leader_map[k] = 'which_key_ignore' end)
